...
public class ArrayExample1 {
	...
	public void ex1( ) {
		...
		// 2-3. 배열 요소 값 대입
		//(int[]) arr = (int) 10; 기본 자료형과 참조형은 서로 대입할 수 없음(형변환 불가) /written by me
		// arr은 int[] 참조형 변수이지만
		// arr[0] int자료형 변수이기 때문에 정수 값을 대입할 수 있다.
	...
	// 0 : int자료형 0 (확실히 존재하는 값)
	
	// "" : String 자료형이지만 내용 없음(빈 문자열, 존재하는 값)
	...
	public void ex5() {
		...
		// char[] arr이 참조하는 배열 요소에 A, B, C, D, E 에 대입하기
		for(int i = 0; i < arr.length; i++) {
			arr[i] = (char)('A' + i);
			// A == 65
			// B == 66
		...
		System.out.println("arr3의 값 : " + Arrays.toString(arr3));
	...
	public void ex7( ) {
		...
		for(int i = 0; i < arr.length; i++ ) {
			...
			// arr[i]에 저장된 값과 input이 같은 경우 
			// 몇 번째 인덱스에 존재하는지? "0번째 인덱스에 존재" 출력
			if(arr[i] == input) {
	...
	public void ex9() {
		...
		String input = sc.nextLine(); // 한 줄 (띄어쓰기 포함) 입력받기
