...
public class BranchExample {
	...
	public void ex2() {
		...
		// 0이 입력될 때 까지의 모든 정수의 합 구하기(while문 사용)
	...
	public void ex3() {
		...
		String str = ""; // 빈 문자열(초기화)
						 // 쌍 따옴표("")라는 기호를 이용해서 String 리터럴임을 지정
						 // 하지만 내용이 없음
		...
		while(true) { // 무한 루프
			...
			if(input.equals("exit!")) {
				...
				// String 자료형은 비교 연산자(==)로 같은 문자열이지 판별할 수 없다.
	...
	public void RPSGame() {
		...
		for(int i = 1; i <= round; i++) {  // 입력 받은 판 수 만큼 반복
			...
			if(input.equals(com)) { // 비긴 경우 
				...
				/*
				 * if(input.equals("가위")&& com.equals("보")) { System.out.println("플레이어 승"); }
				 */
