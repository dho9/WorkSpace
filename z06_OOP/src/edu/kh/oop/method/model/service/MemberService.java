package edu.kh.oop.method.model.service; // 2023-03-03 4번째
...
//  public      static        기본자료형
//  protected   static final  참조형(배열, 클래스)
//  (default)   final         void 
//  private     abstract     
	...
	public String signUp() { // This method must return a result of type String
		...
		if(memberPw.equals(memberPw2)) { // 일치하는 경우
			...
			memberInfo = new Member(memberId, memberPw, memberName, memberAge); //member 자료형이 같음
	...
	public String login() {
		...
		if(memberInfo == null ) { // 회원 가입을 먼저 안한 경우
			...
			return "회원 가입부터 진행 해주세요.";  //if 내에 없으면 return되어서 반환한곳으로 돌아가고 아래 실행X 
		}
		...
		if(memberId.equals(memberInfo.getMemberId()) && 
		   memberPw.equals(memberInfo.getMemberPw())) {
			...
			loginMember = memberInfo; // 로그인 정보에도 가입한 회원정보를 담겠다는 것 
			// 참조형     = Member객체 주소 (얕은 복사) // 주소를 참조하고 있는 상태
	...
	public int updateMember() {
		...
		if(loginMember == null) { //if문이 없으면 바로 반환값이 반환되어버림, 출력구문오류남
		...
		// 2) 수정할 회원 정보 입력 받기
			...
			sc.nextLine(); // 위 입력버퍼에 엔터가 들어가버려서, 하단에 입력되어버림
